...
#include <unistd.h>
EventLoop::EventLoop(bool isSub):ep_(new Epoll),runThreadId_(),eventChannel(new Channel(eventfd(0,EFD_NONBLOCK),ep_.get())),
timeChannel(new Channel(timerfd_create(CLOCK_MONOTONIC,TFD_CLOEXEC|TFD_NONBLOCK),ep_.get())),epollTimeoutCb_(){
    eventChannel->set_et();
    ...
    if(isSub){
        ...
        timeout.it_value.tv_sec = heartBeat;
...
void EventLoop::heart_beat(){
    ...
    for(auto iter:conns_){
        ...
        if((current-connection->lastTime())>connectionTimeout){
